document.addEventListener("DOMContentLoaded", function () {
  ...
  const countryTexts = {
    ...
    // Lütfen diğer tüm ülkelerin metinlerini buraya eklemeyi unutmayın.
  };
  ...
  const fixMap = {
    ...
    china:"cn",        
    australia:"au",    
    unitedstates:"us", // united states (küçük harf ve bitişik)
    "united states":"us", // united states (küçük harf ve boşluklu)
    "united_states":"us", // united_states (küçük harf ve alt çizgi)
    "united states of america":"us", // Daha uzun isim
    "united_states_of_america":"us",
    "united-states":"us",
    "united-states-of-america":"us",
    "united-kingdom":"gb", // İngiltere
    ussr:"ru"          
  };

  // ============================================================
  //  4. TIKLAMA SİSTEMİ (GÜÇLENDİRİLMİŞ EŞLEŞTİRME)
  // ============================================================
  svg.addEventListener("click", function(e){
    ...
    // SVG elementinin ID ve Class değerlerini al, küçük harfe çevir ve normalleştir.
    const idAttr = (target.getAttribute("id") || "").toLowerCase().trim();
    const classAttr = (target.getAttribute("class") || "").toLowerCase().trim();

    // ID ve Class değerlerini, boşlukları ve alt çizgileri kaldırarak ve boşluk bırakarak tek bir dize yapıyoruz.
    const tokens = [
        idAttr,
        ...classAttr.split(/\s+/), // Class içindeki tüm kelimeleri ayrı tokenlar olarak al
        idAttr.replace(/[\s_-]/g, ""), // Boşluksuz/çizgisiz versiyon
        classAttr.replace(/[\s_-]/g, ""),
        idAttr.replace(/\s+/g, " "), // Normal boşluklu hali
        classAttr.replace(/\s+/g, " ")
    ].filter(Boolean);
    ...
    for (const token of uniqueTokens) {
        // 1. FixMap'te tam eşleşme ara (Örn: "united states" -> "us")
        if (fixMap[token]) {
            countryCode = fixMap[token];
            break;
        }
        // 2. ISO 2 harfli kod olarak tam eşleşme ara (Örn: "us" -> "us")
        if (countryNames[token]) {
            countryCode = token;
            break;
        }
    }
    ...
    if (!countryCode) {
        console.warn(`[Tıklama Başarısız]: Ülke kodu hiçbir yöntemle bulunamadı. ID: "${idAttr}", Class: "${classAttr}"`);
        return;
    }
    ...
    modalVideo.play().catch(error => {
        console.log(`Video otomatik oynatılamadı. Kod: ${countryCode}`, error);
    });
